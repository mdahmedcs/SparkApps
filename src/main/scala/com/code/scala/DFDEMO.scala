package src.main.scala.com.code.scala
...
object DFDEMO {
  ...
  def main(args:Array[String]): Unit={
     ...
     val log: Logger = Logger.getLogger("DFDEMO.scala")
...
//  import spark.sqlContext.implicits._
 ...
 //withColumn 
 println("withcolumn demo")
 val DF2=DF.withColumn("multiplication", col("salary")*100)
 DF2.show()
 
 println("withColum date and timestamp")
 DF2.withColumn("date", current_date()).withColumn("timestamp", from_unixtime(unix_timestamp(),"yyyy-MM-dd HH:mm:ss")).show(false)
 ...
 //withColumn using when otherwise
  ...
  //applying hash
  println("applying hash")
  DF.withColumn(("hashname"), hash  (col("name"))).show()
  
  //applying hashcode
  println("applying hash")
  DF.withColumn(("hashname"), col("name")).show()
  
  //rank and dense_rank, percent_rank function
  println("rank")
  DF.withColumn("rank", rank().over(Window.orderBy("salary"))).show(false)
  
  println("dense_rank")
  DF.withColumn("rank", dense_rank().over(Window.orderBy("salary"))).show(false)
  
  println("percent_rank")
  DF.withColumn("rank", percent_rank().over(Window.orderBy("salary"))).show(false)
...
//inner join
 ...
 println("applying udf")
 val sampleudf=spark.read.options(Map("inferschema"->"true", "header"->"true")).csv("E:\\spark_examples\\subscribers.csv")
 

 // spark.udf.register("myUpper", (input: String) => input.toUpperCase)
// val udfupper = udf("myUpper")
 
// sampleudf.withColumn("cap_email", udfupper(col("email"))).select("email", "cap_email").show(false)                    
